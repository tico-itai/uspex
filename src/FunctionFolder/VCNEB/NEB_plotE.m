...
fclose(fp);
[nothing, nothing] = unix( ['sed -i "s/0/ /g" ',POP_STRUC.resFolder,'/BarrierFig'] );
[nothing, nothing] = unix( ['sed -i "s/1/+/g" ',POP_STRUC.resFolder,'/BarrierFig'] );
[nothing, nothing] = unix( ['sed -i "s/2/|/g" ',POP_STRUC.resFolder,'/BarrierFig'] );
fp=fopen(fpath,'a');
for i=1:numImages
fprintf(fp,'%3s','--o');
end
...
fclose(fp);
[nothing, nothing] = unix(['sed -i "s/STEPNUM/' num2str(POP_STRUC.step) '/g" ' POP_STRUC.resFolder,'/BarrierFig']);
