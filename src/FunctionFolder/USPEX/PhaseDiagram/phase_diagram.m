...
enth_list,   ...
pressures_list];
init_matrix_IVEP = sortrows(init_matrix_IVEP, 2);   
...
pos = 1;
matrix_IVEP = sortrows(matrix_IVEP, 3);    
else
pos = size(matrix_IVEP, 1);
matrix_IVEP = sortrows(matrix_IVEP, 4);    
end
...
all_values_by_volume = sortrows([pos_phase_transforms([2:end],:); neg_phase_transforms], 2); 
ids      = all_values_by_volume(:, 1);
volume   = all_values_by_volume(:, 2);
enthalpy = all_values_by_volume(:, 3);
pressure = all_values_by_volume(:, 4);
figure('Name','Pressures output', 'NumberTitle','off'); 
set(gcf,'Visible','off');   
plot(...
volume, enthalpy,...
'Color','blue',...
