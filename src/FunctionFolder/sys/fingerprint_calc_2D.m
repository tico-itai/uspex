...
fing((typ_i(i)-1)*species+typ_j(j), bins) = fing((typ_i(i)-1)*species+typ_j(j), bins) + delt/(2*R0^2); 
elseif abs(R0) < up && abs(R0) > down
fing((typ_i(i)-1)*species+typ_j(j), bins) = fing((typ_i(i)-1)*species+typ_j(j), bins) + delt/(R0^2+R0*down); 
elseif abs(R0) < down && abs(R0) > up
fing((typ_i(i)-1)*species+typ_j(j), bins) = fing((typ_i(i)-1)*species+typ_j(j), bins) + delt/(R0^2+R0*up); 
elseif abs(R0) > down && abs(R0) > up
fing((typ_i(i)-1)*species+typ_j(j), bins) = fing((typ_i(i)-1)*species+typ_j(j), bins) + delt/(R0*(up+down)); 
end
end
end
