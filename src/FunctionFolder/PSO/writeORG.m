...
fprintf(fp,'\n');
for i=1:length(ORG_STRUC.atomType)
fprintf(fp,'    Minimum distances:                  %4s: ', megaDoof(ceil(ORG_STRUC.atomType(i))));
fprintf(fp,'%4.2f %4.2f %4.2f\n', ORG_STRUC.minDistMatrice(i,:));
end
fprintf(fp,'\n');
if ORG_STRUC.molecule==1
fprintf(fp,'    There are%2d types of molecules in the system: ', size(ORG_STRUC.numMols,2));
for i=1:size(ORG_STRUC.numIons,2)
fprintf(fp,'%6s,', ORG_STRUC.STDMOL(i).name);
end
fprintf(fp,'\n');
