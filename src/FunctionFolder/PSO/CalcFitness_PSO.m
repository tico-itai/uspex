function fitness = CalcFitness_PSO()
global POP_STRUC
global ORG_STRUC
...
for fit_loop = 1:length(POP_STRUC.POPULATION)
fitness(fit_loop) = -POP_STRUC.POPULATION(whichInd).S_order;
end
...
for fit_loop = 1 : length(POP_STRUC.POPULATION)
fitness(fit_loop) = -1*POP_STRUC.struc_entr(fit_loop);
end
elseif (ORG_STRUC.optType > 1100) & (ORG_STRUC.optType < 1112)  
whichPara= mod(ORG_STRUC.optType,110);
for i = 1 : length(POP_STRUC.POPULATION)
if isempty(POP_STRUC.POPULATION(i).elasticProperties)
fitness(i)=NaN;
else
