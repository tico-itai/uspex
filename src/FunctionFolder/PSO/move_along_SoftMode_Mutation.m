...
if step > 10
% [nothing, nothing] = unix(['echo main_structure >> POSCAR_cluster']);
% [nothing, nothing] = unix(['echo ' '1.0' ' >> POSCAR_cluster']);
%  [nothing, nothing] = unix(['echo ' num2str(lattice(latticeLoop,:)) ' >> POSCAR_cluster']);
% [nothing, nothing] = unix(['echo ' num2str(numIons) ' >> POSCAR_cluster']);
% [nothing, nothing] = unix(['echo ' 'Direct' ' >> POSCAR_cluster']);
%  [nothing, nothing] = unix(['echo ' num2str(coord(coordLoop,:)) ' >> POSCAR_cluster']);
notDone = 0;
end
end
