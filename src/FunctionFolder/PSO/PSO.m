...
for ind = 1 : ORG_STRUC.populationSize
dist1 = cosineDistance(POP_STRUC.POPULATION(ind).FINGERPRINT, POP_STRUC.PSO(ind).fingerprint, ORG_STRUC.weight);   
dist2 = cosineDistance(POP_STRUC.POPULATION(ind).FINGERPRINT, POP_STRUC.PSO(POP_STRUC.bestPSOstruc).fingerprint, ORG_STRUC.weight); 
...
if rand < P_r | POP_STRUC.POPULATION(ind).Error > ORG_STRUC.maxErrors 
Random_PSO(ind);
else
